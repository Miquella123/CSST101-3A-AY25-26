...
# ---------- Rule 5: Wi-Fi Access ---------- #
def wifi_access_rule(student_name):
    print("\n--- Wi-Fi Access Rule Checker ---")
    enrolled = input("Is the student enrolled? (T/F): ").strip().upper() == "T"
    password = input("Enter Wi-Fi password: ")
    valid_password = (password == "wifi2025")

    access_granted = enrolled and valid_password
    outcome = "Wi-Fi Access Granted" if access_granted else "Wi-Fi Access Denied"

    print(f"P = {tf(enrolled)} (Enrolled), Q = {tf(valid_password)} (Password Valid)")
    print("Result:", outcome)

    log_result(student_name, "Wi-Fi Access Rule", outcome)
...
def main():
    ...
    while True:
        ...
        print("5) Wi-Fi Access Rule Checker")
        print("6) Exit")
        ...
        match choice:
            ...
            case "5": wifi_access_rule(student_name)
            case "6":
